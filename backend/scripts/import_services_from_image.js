...
const generatePayload = () => {
    ...
    servicesData.forEach(group => {
        group.prices.forEach((price, index) => {
            ...
            payload.push({
                name: `${group.name} - ${size} (${group.category})`,
...
const runImport = async () => {
    ...
    console.log(`Prepared ${payload.length} services for import...`);

    try {
        const response = await axios.post('http://localhost:3001/services/bulk', payload);
