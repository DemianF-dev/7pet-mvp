            ...
            <div className="space-y-3">
                {statuses.map((s) => (
                    <button
                        key={s}
                        onClick={() => setStatus(s)}
                        className={`w-full py-4 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all text-left flex items-center justify-between ${status === s ? 'bg-primary text-white shadow-lg' : 'bg-white/5 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 hover:bg-white/10 dark:hover:bg-gray-700'}`}
                    >
                        {s}
