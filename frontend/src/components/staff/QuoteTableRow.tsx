...
const QuoteTableRow = React.memo(({
    ...
    return (
        <tr className={`hover:bg-gray-50/50 transition-all group ${isSelected ? 'bg-primary/5' : ''}`}>
                ...
                <div className="flex justify-end gap-2">
                    {view === 'trash' ? (
                        <>
                            ...
                            <button onClick={() => onPermanentDelete(quote.id)} className="p-2 hover:bg-red-50 text-red-500 rounded-xl transition-all" title="Excluir Permanente"><Trash2 size={18} /></button>
                        </>
                    ) : view === 'history' ? (
                        <>
                            <button onClick={() => onDuplicate(quote.id)} className="p-2 hover:bg-blue-50 text-blue-500 rounded-xl transition-all opacity-0 group-hover:opacity-100" title="Duplicar Orçamento"><Copy size={18} /></button>
                            <button
                            ...
                            <button onClick={() => onViewDetails(quote.id)} className="p-2 bg-gray-50 hover:bg-gray-100 text-secondary rounded-xl transition-all" title="Ver Detalhes"><Edit size={18} /></button>
                        </>
                    ) : (
                        <>
                            <button onClick={() => onDuplicate(quote.id)} className="p-2 hover:bg-blue-50 text-blue-500 rounded-xl transition-all opacity-0 group-hover:opacity-100" title="Duplicar"><Copy size={18} /></button>
                            <button onClick={() => onShare(quote)} className="p-2 hover:bg-green-50 text-green-600 rounded-xl transition-all opacity-0 group-hover:opacity-100" title="Compartilhar no WhatsApp"><Share2 size={18} /></button>
                            <button onClick={() => onDelete(quote.id)} className="p-2 hover:bg-red-50 text-red-500 rounded-xl transition-all opacity-0 group-hover:opacity-100" title="Mover para Lixeira"><Trash2 size={18} /></button>
                            <button onClick={() => onViewDetails(quote.id)} className="p-2 bg-gray-50 hover:bg-gray-100 text-secondary rounded-xl transition-all" title="Ver Detalhes"><Edit size={18} /></button>
