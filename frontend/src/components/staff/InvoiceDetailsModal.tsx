...
export default function InvoiceDetailsModal({ invoiceId, onClose, onUpdate }: Props) {
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="bg-indigo-50 border border-indigo-100 rounded-3xl p-6">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <span className="bg-indigo-200 text-indigo-700 text-[10px] font-black px-2 py-1 rounded-lg uppercase">
                                        RECORRENTE
                                    </span>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    >
                                        <span className="text-[10px] font-black uppercase">Início do Pacote</span>
                                    ...
                                    >
                                        <span className="text-[10px] font-black uppercase">Final do Pacote</span>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100 flex flex-col">
                                    <span className="text-[10px] font-black text-gray-400 uppercase mb-2">Valor Total</span>
                                    <p className="text-2xl font-black text-secondary">R$ {invoice.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                </div>
                                <div className="bg-emerald-50 p-5 rounded-3xl border border-emerald-100 flex flex-col">
                                    <span className="text-[10px] font-black text-emerald-600 uppercase mb-2">Recebido</span>
                                    <p className="text-2xl font-black text-emerald-700">R$ {totalPaid.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                </div>
                                <div className={`${remaining > 0 ? 'bg-orange-50 border-orange-100' : 'bg-emerald-50 border-emerald-100'} p-5 rounded-3xl border flex flex-col`}>
                                    <span className={`text-[10px] font-black ${remaining > 0 ? 'text-orange-600' : 'text-emerald-600'} uppercase mb-2`}>Pendente</span>
                                    <p className={`text-2xl font-black ${remaining > 0 ? 'text-orange-700' : 'text-emerald-700'}`}>R$ {remaining.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                </div>
                            </div>

                            {/* Linked Items (Quotes/Appointments) */}
                            <div>
                                <h3 className="text-sm font-black text-secondary uppercase tracking-widest mb-4">Itens Vinculados</h3>
                                    ...
                                    {invoice.quotes?.map(q => (
                                        <div key={q.id} className="bg-white border border-gray-100 p-4 rounded-2xl flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-xs font-black text-secondary uppercase">Orçamento #{String(q.seqId).padStart(4, '0')}</p>
                                                    <p className="text-[10px] text-gray-400 font-bold">{q.items?.length || 0} Itens</p>
                                                </div>
                                            </div>
                                            <p className="font-black text-secondary">R$ {q.totalAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                        </div>
                                    ))}
                                    {invoice.appointment && (
                                        <div className="bg-white border border-gray-100 p-4 rounded-2xl flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-xs font-black text-secondary uppercase tracking-tight">Agendamento #{String(invoice.appointment.seqId).padStart(4, '0')}</p>
                                                    <p className="text-[10px] text-gray-400 font-bold">Pet: {invoice.appointment.pet?.name}</p>
                                                </div>
                                            </div>
                                            <p className="font-black text-secondary">Vínculo Direto</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* Payment History */}
                            <div>
                                <h3 className="text-sm font-black text-secondary uppercase tracking-widest mb-4">Histórico de Pagamentos</h3>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <button
                                                            ...
                                                            className="text-sm font-black text-secondary hover:text-primary transition-colors text-left"
                                                        >
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Ações de Status</h3>
                                <div className="grid grid-cols-1 gap-2">
                                    {['PENDENTE', 'PAGO', 'MONITORAR', 'FATURAR', 'VENCIDO'].map(st => (
                                        <button
                                            key={st}
                                            onClick={() => handleUpdateStatus(st)}
                                            className={`px-4 py-3 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest transition-all ${invoice.status === st ? getStatusColor(st) : 'bg-white border-gray-50 text-gray-400 hover:border-gray-100'}`}
                                        >
                            ...
                            {/* Quick Add Payment */}
                            <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Registrar Recebimento</h3>
                                <form onSubmit={handleRegisterPayment} className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-black focus:ring-2 focus:ring-primary focus:border-transparent outline-none"
                                            placeholder="R$ 0,00"
                                            required
                                        />
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-gray-400 uppercase mb-1 block">Método</label>
                                        <select
                                            value={paymentMethod}
                                    ...
                                    <button
                                        type="submit"
                                        className="w-full bg-primary text-white py-3 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                                    >
                                        Confirmar
                                    </button>
                                </form>
                            ...
                            {/* Internal Notes */}
                            <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Observações Internas</h3>
                                <textarea
                                    value={notes}
                                ...
                                <button
                                    ...
                                    className="w-full mt-3 bg-secondary text-white py-2 rounded-xl font-black text-[10px] uppercase tracking-widest hover:opacity-90 transition-all flex items-center justify-center gap-2"
                                >
