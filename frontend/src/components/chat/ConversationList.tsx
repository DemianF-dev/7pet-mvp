        ...
        <div className="w-full h-full overflow-y-auto p-2 space-y-1 bg-gray-50 dark:bg-gray-900/50">
            ...
            {conversations.map(conv => {
                ...
                return (
                    ...
                    >
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold shrink-0 transition-colors ${isActive ? 'bg-blue-600 text-white' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                            }`}>
                            {name[0]?.toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start">
                                <span className={`font-semibold text-sm truncate ${isActive ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-200'}`}>
                                    {name}
                                </span>
                                {/* Timestamp placeholder if available */}
                            </div>
                            <div className={`text-xs truncate ${isActive ? 'text-gray-500' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300'}`}>
                                {lastMsg}
                            </div>
                        </div>
                    </button>
