...
import { X, Plus, Trash2, Search, Truck, MapPin, RefreshCcw, Scissors, Droplets, Bug, AlertTriangle, Calendar, Minus, CheckCircle, Info } from 'lucide-react';
...
export default function ManualQuoteModal({ isOpen, onClose, onSuccess }: ManualQuoteModalProps) {
    ...
    const recalculateTransport = (info: any) => {
        ...
        if (info.legs.largada.active) {
            base += (Number(info.legs.largada.km) * settings.kmPriceLargada) + (Number(info.legs.largada.min) * settings.minPriceLargada);
        }
        // Pernada 2: Leva
        ...
        if (info.legs.retorno.active) {
            base += (Number(info.legs.retorno.km) * settings.kmPriceRetorno) + (Number(info.legs.retorno.min) * settings.minPriceRetorno);
        }
    ...
    const getCategorizedProducts = () => {
        ...
        // Ordenar produtos dentro das categorias
        Object.keys(cats).forEach(k => {
            cats[k].sort((a, b) => a.name.localeCompare(b.name));
        });

        return cats; // Retorna objeto { 'Higiene': [...], 'Brinquedos': [...] }
    };
    ...
    const totalBase = quote.items.reduce((acc, item) => acc + (item.price * item.quantity), 0);
    const discountRate = getDiscountRate();
    const totalDiscount = quote.items.reduce((acc, item) => {
        // Appy discount only to services from the catalog (with serviceId)
        if (item.serviceId && discountRate > 0) {
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-8 bg-gray-50/30 dark:bg-gray-900/50">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            className="w-full bg-orange-50/50 border-none rounded-2xl px-5 py-4 font-bold text-secondary outline-none focus:ring-2 focus:ring-orange-500/20"
                                                        />
                                                    </div>
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            className="w-full bg-teal-50/50 border-none rounded-2xl px-5 py-4 font-bold text-secondary outline-none focus:ring-2 focus:ring-teal-500/20"
                                                        />
                                                    </div>
                                                </div>
                                                ...
                                                {transportInfo && transportInfo.legs && (
                                                    <div className="mt-4 space-y-4 animate-in slide-in-from-left">
                                                        ...
                                                        <div className="space-y-3">
                                                            {[
                                                                { id: 'largada', label: '🚀 Largada (Loja -> Coleta)', color: 'bg-blue-50 text-blue-600' },
                                                                { id: 'leva', label: '📦 Leva (Coleta -> Loja)', color: 'bg-green-50 text-green-600' },
                                                                { id: 'traz', label: '🏠 Traz (Loja -> Entrega)', color: 'bg-purple-50 text-purple-600' },
                                                                { id: 'retorno', label: '🔄 Retorno (Entrega -> Loja)', color: 'bg-orange-50 text-orange-600' }
                                                            ].map(leg => (
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="relative">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={e => setTransportInfo({ ...transportInfo, estimatedPrice: e.target.value === '' ? '' : (parseFloat(e.target.value) || 0) })}
                                                            ...
                                                            {transportInfo.settings && (
                                                                <div className="px-3 py-2 bg-blue-100/50 dark:bg-blue-900/30 rounded-xl text-[8px] leading-tight space-y-1">
                                                                    {quote.petQuantity > 1 && <div className="font-bold text-blue-600 dark:text-blue-300">+{(quote.petQuantity - 1) * 20}% Pets Adic.</div>}
                                                                    <div className="flex items-center gap-1">
                                                                        <span className="text-blue-500">R$/KM:</span>
                                                                        <input
                                                                            type="number"
                                                                            step="0.1"
                                                                            value={((transportInfo.settings.kmPriceLeva + transportInfo.settings.kmPriceTraz) / 2).toFixed(2)}
                                                                            onChange={e => {
                                                                            ...
                                                                            className="w-14 text-[10px] font-black text-blue-700 dark:text-blue-300 bg-white dark:bg-gray-700 rounded px-1 py-0.5 outline-none border border-blue-200 dark:border-blue-800"
                                                                        />
                                                                    </div>
                                                                    <div className="flex items-center gap-1">
                                                                        <span className="text-blue-500">R$/Min:</span>
                                                                        <input
                                                                            type="number"
                                                                            step="0.1"
                                                                            value={((transportInfo.settings.minPriceLeva + transportInfo.settings.minPriceTraz) / 2).toFixed(2)}
                                                                            onChange={e => {
                                                                                const val = parseFloat(e.target.value) || 0;
                                                                                const next = {
                                                                                    ...transportInfo,
                            ...
                            <section className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-6">
                                                        ...
                                                        <>
                                                            ...
                                                            >
                                                                ...
                                                                {(() => {
                                                                    ...
                                                                    const sortedKeys = Object.keys(prodCats).sort();

                                                                    return sortedKeys.map(catName => (
                                                                        <optgroup key={catName} label={`📦 PRODUTOS: ${catName.toUpperCase()}`}>
                                                                            {prodCats[catName].map(p => (
                                ...
                                {/* Resumo e Total */}
                                <div className="mt-8 pt-8 border-t border-gray-50 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-6">
                                        ...
                                        <div className="flex flex-col items-end mb-1">
                                            ...
                                            {totalDiscount > 0 && (
                                                <span className="text-[10px] font-black text-green-500 uppercase">
                                                    Recorrência {customer.recurrenceFrequency}: - R$ {totalDiscount.toFixed(2)}
                                                </span>
                                            )}
                                        </div>
