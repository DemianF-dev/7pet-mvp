...
export const UserFormModal: React.FC<UserFormModalProps> = ({
    ...
    const isTargetHighPrivilege = selectedUser && (selectedUser.role?.toUpperCase() === 'ADMIN' || selectedUser.role?.toUpperCase() === 'MASTER');
    const isAdmin = currentUser?.role?.toUpperCase() === 'ADMIN';
    const canEdit = isMaster || (isAdmin && selectedUser?.role?.toUpperCase() !== 'MASTER');
    const isProtected = !!(isTargetHighPrivilege && !canEdit);
            ...
            >
                ...
                {isProtected && (
                    <div className="bg-red-500 text-white p-3 text-center text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                        <Shield size={14} /> Somente o Master {isAdmin ? 'pode editar usuários Master' : 'pode editar perfis Administrativos ou Master'}
                    </div>
                )}
                    ...
                    {formData.division !== 'CLIENTE' && (
                        <div className="pt-4 border-t border-gray-50 mt-4">
                            ...
                            <label className="flex items-center gap-3 p-4 bg-orange-50/50 rounded-2xl cursor-pointer hover:bg-orange-50 transition-colors border border-orange-100 mt-3">
                                ...
                                />
                                <div className="flex-1">
                                    <div className="text-sm font-bold text-secondary uppercase tracking-tight">Permitir uso como Cliente</div>
                                ...
                                <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${formData.allowCustomerProfile ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'}`}>
