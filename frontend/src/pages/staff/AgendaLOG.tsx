...
export default function AgendaLOG() {
    ...
    const updateStatus = async (id: string, status: string, e?: React.MouseEvent) => {
        ...
        const statusLabels: any = {
            ...
            'EM_ATENDIMENTO': 'iniciar rota para',
                ...
                <div className="flex flex-col min-h-full">
                    {timeSlots.map((slot) => (
                        <div key={slot} className="flex border-b border-gray-50 group min-h-[45px]">
                            ...
                            <div className="flex-1 relative p-1 bg-white group-hover:bg-orange-50/30 transition-colors">
                                {dayAppointments
                                    .filter(a => {
                                    ...
                                    .map(appt => (
                                        <div
                                            ...
                                            className="px-4 py-2 bg-orange-50 hover:bg-orange-100/50 border-l-4 border-orange-500 rounded-xl cursor-pointer transition-all mb-1 relative group overflow-hidden shadow-sm"
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="ml-auto bg-orange-100 text-orange-600 text-[8px] font-black px-2 py-0.5 rounded-full uppercase">{appt.status}</span>
                                            </div>
                                        </div>
                                    ))}
                            </div>
                ...
                <div className="flex-1 overflow-auto custom-scrollbar relative">
                    <div className="min-w-[1000px]">
                        <div className="flex border-b border-gray-100 bg-gray-50/50 sticky top-0 z-20">
                            ...
                            {weekDays.map(d => (
                                <div key={d.toString()} className={`flex-1 p-3 text-center border-r border-gray-100 last:border-none ${isSameDay(d, new Date()) ? 'bg-orange-50' : ''}`}>
                                    <p className={`text-[10px] font-black uppercase ${isSameDay(d, new Date()) ? 'text-orange-600' : 'text-gray-400'}`}>{d.toLocaleDateString('pt-BR', { weekday: 'short' })}</p>
                                    <p className={`text-lg font-black ${isSameDay(d, new Date()) ? 'text-orange-600' : 'text-secondary'}`}>{d.getDate()}</p>
                                </div>
                            ))}
                        </div>
                        ...
                        {timeSlots.map(slot => (
                            <div key={slot} className={`flex border-b border-gray-50 group min-h-[40px]`}>
                                ...
                                {weekDays.map(day => (
                                    <div key={day.toString()} className={`flex-1 border-r border-gray-50 last:border-none p-0.5 relative hover:bg-orange-50/20 transition-colors ${isSameDay(day, new Date()) ? 'bg-orange-50/10' : ''}`}>
                                        {filteredAppointments
                                            ...
                                            .map(appt => (
                                                <div
                                                    key={appt.id}
                                                    onClick={() => handleOpenDetails(appt)}
                                                    className="p-1 bg-orange-50 hover:bg-orange-100/50 border-l-2 border-orange-500 rounded-lg cursor-pointer transition-all mb-0.5 shadow-sm group overflow-hidden"
                                                >
                                                    <div className="flex flex-col gap-0.5">
                                                        <div className="flex items-center justify-between gap-1 overflow-hidden">
                                                            <span className="text-[8px] font-black text-secondary truncate uppercase">{appt.pet.name}</span>
                                                            <span className="text-[7px] font-black text-orange-600 shrink-0">
                                                                {new Date(appt.startAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                                                            </span>
                                                        </div>
                                                    </div>
                        ...
                        {days.map((day, idx) => {
                            ...
                            return (
                                <React.Fragment key={idx}>
                                    {weekNum !== null && (
                                        <div className="border-r border-b border-gray-100 flex flex-col items-center justify-start pt-4 bg-orange-50/5 text-[12px] font-black text-orange-200">
                                            {weekNum}
                                        </div>
                                    )}
                                    <div
                                        ...
                                        <div className="space-y-[4px]">
                                            {dayAppts.slice(0, 10).map(appt => {
                                                ...
                                                return (
                                                    <div
                                                        key={appt.id}
                                                        onClick={() => handleOpenDetails(appt)}
                                                        className="flex items-center gap-2 group/item cursor-pointer hover:bg-orange-50 p-1 rounded-md transition-all"
                                                    >
                                            ...
                                            {dayAppts.length > 10 && (
                                                <button
                                                    ...
                                                    className="w-full mt-1 py-1 text-[9px] font-black text-orange-500/60 hover:text-orange-500 uppercase flex items-center justify-center gap-1 hover:bg-orange-500/5 rounded-md transition-all border border-transparent hover:border-orange-500/20"
                                                >
            ...
            <main className="flex-1 md:ml-64 p-4 md:p-8">
                ...
                <header className="mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex items-center gap-8">
                        <div>
                            <div className="flex items-center gap-3 text-orange-500 font-black text-[10px] uppercase tracking-[0.3em] mb-3">
                        ...
                        <div className="flex bg-white/80 backdrop-blur-md p-1.5 rounded-2xl shadow-sm border border-gray-100 mt-6 lg:mt-0">
                            <button
                                ...
                                className="p-2 hover:bg-gray-50 rounded-xl transition-all text-gray-400 hover:text-orange-500"
                            >
                                <ChevronLeft size={20} />
                            </button>
                            <button
                                onClick={() => setSelectedDate(new Date())}
                                className="px-6 py-2 text-[11px] font-black text-secondary hover:text-orange-500 uppercase tracking-[0.2em] transition-colors"
                            >
                                Hoje
                            </button>
                            <button
                    ...
                    <div className="flex flex-wrap items-center gap-4 bg-white/50 backdrop-blur-md p-2 rounded-[28px] border border-white shadow-xl shadow-black/5">
                        <div className="flex items-center gap-1 bg-gray-100/50 p-1 rounded-[22px]">
                            {['KANBAN', 'DAY', 'WEEK', 'MONTH'].map((v) => (
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-6 py-3 rounded-[18px] text-[10px] font-black transition-all ${view === v ? 'bg-white text-orange-500 shadow-lg shadow-black/10 scale-[1.02]' : 'text-gray-400 hover:text-secondary'}`}
                                >
                        ...
                        <button
                            ...
                            className="bg-orange-500 hover:bg-orange-600 text-white px-8 py-4 rounded-[22px] font-black shadow-xl shadow-orange-500/30 hover:shadow-orange-500/40 active:scale-95 transition-all text-[11px] tracking-[0.1em] flex items-center gap-3 uppercase"
                        >
                ...
                <div className="flex flex-wrap items-center gap-4 mb-8">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="bg-white border-transparent border-2 rounded-[22px] pl-12 pr-6 py-3.5 text-sm shadow-sm focus:border-orange-500/20 focus:ring-0 w-[400px] transition-all"
                        />
                    </div>
                    ...
                    <button
                        ...
                        disabled={isLoading}
                        className="p-3.5 bg-white text-gray-400 rounded-[18px] border border-gray-100 shadow-sm hover:text-orange-500 hover:border-orange-500/30 transition-all active:scale-90 disabled:opacity-50"
                ...
                {!isTrashView ? (
                                        ...
                                        <AnimatePresence mode="popLayout">
                                            {getColumnItems(col.key).map((appt) => (
                                                <motion.div
                                                    ...
                                                    className="bg-white p-5 rounded-[28px] shadow-sm border border-gray-100 group hover:shadow-2xl hover:shadow-orange-500/10 hover:border-orange-500/30 transition-all cursor-pointer relative overflow-hidden active:scale-[0.98]"
                                                >
                                                    <div className="flex justify-between items-start mb-4">
                                                        <div className="bg-orange-50 px-3 py-1.5 rounded-xl flex items-center gap-2 text-[10px] font-black text-orange-600 border border-orange-100">
                                                    ...
                                                    <div className="space-y-1 mb-5">
                                                        <h4 className="font-black text-secondary text-sm group-hover:text-orange-600 transition-colors truncate uppercase tracking-tight">
                                                            {appt.pet.name}
                                                        </h4>
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        {col.key === 'CONFIRMADO' && (
                                                            <button
                                                                ...
                                                                className="flex-1 py-2.5 bg-orange-500 text-white rounded-[14px] text-[10px] font-black flex items-center justify-center gap-2 hover:bg-orange-600 transition-all shadow-lg shadow-orange-500/20 active:scale-95"
                                                            >
                    ...
                    <div className="bg-white rounded-[40px] p-10 shadow-sm border border-gray-100 min-h-[500px]">
                        <div className="flex items-center gap-6 mb-10 pb-10 border-b border-gray-50">
                            ...
                            <div>
                                <h2 className="text-3xl font-black text-secondary tracking-tight">Lixeira Logística</h2>
