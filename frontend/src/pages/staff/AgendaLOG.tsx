                ...
                <div className="flex flex-col min-h-full">
                    {timeSlots.map((slot) => (
                        <div key={slot} className="flex border-b border-gray-50 group min-h-[45px]">
                            ...
                            <div className="flex-1 relative p-1 bg-white group-hover:bg-orange-50/50 transition-colors">
                                {dayAppointments
                                    .filter(a => {
                                    ...
                                    .map(appt => (
                                        <div
                                            ...
                                            className="px-4 py-2 bg-orange-50 hover:bg-orange-100 border-l-4 border-orange-500 rounded-xl cursor-pointer transition-all mb-1 relative group overflow-hidden shadow-sm"
                                        >
        ...
        return (
            <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[calc(100vh-220px)]">
                <div className="grid grid-cols-8 border-b border-gray-100 bg-gray-50/50">
                    ...
                    {weekDays.map(day => (
                        <div key={day.toISOString()} className={`p-4 text-center border-r border-gray-100 last:border-none ${isSameDay(day, new Date()) ? 'bg-orange-50' : ''}`}>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{day.toLocaleDateString('pt-BR', { weekday: 'short' })}</p>
                            <p className={`text-xl font-black ${isSameDay(day, new Date()) ? 'text-orange-500' : 'text-secondary'}`}>{day.getDate()}</p>
                        </div>
                    ))}
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar relative">
                    {timeSlots.map(slot => (
                        <div key={slot} className="grid grid-cols-8 border-b border-gray-50 group min-h-[45px]">
                            ...
                            {weekDays.map(day => (
                                <div key={day.toISOString()} className={`border-r border-gray-50 last:border-none p-1 relative min-h-[45px] group-hover:bg-gray-50/30 transition-colors ${isSameDay(day, new Date()) ? 'bg-orange-50/30' : ''}`}>
                                    {filteredAppointments
                                        ...
                                        .map(appt => (
                                            <div
                                                key={appt.id}
                                                onClick={() => handleOpenDetails(appt)}
                                                className="p-2 bg-orange-500 text-white rounded-xl cursor-pointer shadow-lg shadow-orange-500/20 hover:scale-[1.02] transition-all mb-1"
                                            >
                                    ...
                                    >
                                        ...
                                        <div className="space-y-[4px]">
                                            {dayAppts.slice(0, 5).map(appt => {
                                                ...
                                                const dotColor = appt.status === 'PENDENTE' ? 'bg-orange-500' :
                                            ...
                                            {dayAppts.length > 5 && (
                                                <button
                                                    ...
                                                    className="w-full mt-1 py-1 text-[9px] font-black text-orange-500/60 hover:text-orange-500 uppercase flex items-center justify-center gap-1 hover:bg-orange-50 rounded-md transition-all"
                                                >
            ...
            <main className="flex-1 md:ml-64 p-4 md:p-8">
                <header className="mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex items-center gap-8">
                        ...
                        <div className="flex bg-white/80 backdrop-blur-md p-1.5 rounded-2xl shadow-sm border border-gray-100 mt-6 lg:mt-0">
                            <button onClick={prevDay} className="p-2 hover:bg-gray-50 rounded-xl transition-all text-gray-400 hover:text-orange-500">
                                <ChevronLeft size={20} />
                            </button>
                            <button onClick={setToday} className="px-6 py-2 text-[11px] font-black text-secondary hover:text-orange-500 uppercase tracking-[0.2em] transition-colors">
                                Hoje
                            </button>
                            <button onClick={nextDay} className="p-2 hover:bg-gray-50 rounded-xl transition-all text-gray-400 hover:text-orange-500">
                    ...
                    <div className="flex flex-wrap items-center gap-4 bg-white/50 backdrop-blur-md p-2 rounded-[28px] border border-white shadow-xl shadow-black/5">
                        ...
                        <button
                            ...
                            className="bg-orange-500 hover:bg-orange-600 text-white px-8 py-4 rounded-[22px] font-black shadow-xl shadow-orange-500/30 transition-all text-[11px] tracking-[0.1em] flex items-center gap-3 uppercase"
                        >
                ...
                <div className="relative mb-8">
                    ...
                    <input
                        ...
                        className="w-full bg-white border-none rounded-[32px] pl-16 pr-8 py-5 text-sm shadow-sm focus:ring-2 focus:ring-orange-500/20 transition-all font-medium"
                    />
                </div>
                                        ...
                                        <div className="flex-1 bg-gray-50/50 rounded-[40px] p-5 overflow-y-auto space-y-4 custom-scrollbar border border-dashed border-gray-200">
                                            {getColumnItems(col.key).map((appt) => (
                                                <div
                                                    key={appt.id}
                                                    onClick={() => handleOpenDetails(appt)}
                                                    className="bg-white p-5 rounded-[28px] shadow-sm border border-gray-100 group hover:shadow-2xl hover:shadow-orange-500/10 transition-all cursor-pointer relative overflow-hidden"
                                                >
                                                    ...
                                                    <h4 className="font-black text-secondary text-sm group-hover:text-orange-500 transition-colors truncate uppercase">{appt.pet.name}</h4>
