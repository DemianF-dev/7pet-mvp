...
import { User, Save, ArrowLeft, Building2, User as UserIcon, CreditCard, FileText, Heart, Shield, MapPin, Truck } from 'lucide-react';
        ...
        <main className="p-4 md:p-8 max-w-5xl mx-auto w-full" style={{ paddingBottom: 'calc(140px + env(safe-area-inset-bottom, 24px))' }}>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 mb-8">
                ...
                <div className="text-center md:text-left">
                    <h1 className="text-xl md:text-2xl font-bold text-heading">{profile.user.name}</h1>
            ...
            <div className="flex gap-4 mb-6 border-b border-slate-200">
                <button
                    ...
                    className={`pb-3 px-2 text-sm font-bold transition-colors border-b-2 ${activeTab === 'details' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                    Dados Cadastrais
                </button>
                <button
                    onClick={() => setActiveTab('financial')}
                    className={`pb-3 px-2 text-sm font-bold transition-colors border-b-2 ${activeTab === 'financial' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                    Contrato & Financeiro
                </button>
            </div>
            ...
            <style>{`
                    .label-sm {
                        @apply text-xs font-bold text-muted uppercase tracking-wider mb-2 block;
                    }
                    .input-field {
